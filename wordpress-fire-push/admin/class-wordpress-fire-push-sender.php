    ...
    public function send($roles = 'all')
    {
        ...
        foreach ($registration_ids as $registration_id) {
            ...
            if($result['success'] > 0) {
                ...
                $notifications[] = $result['multicast_id'];
                
                update_option('fire_push_notifications', $notifications);

                $notification = array_merge($data, $result);

                add_option('fire_push_notification_' . $multicast_id, $notification);
                ...
                if($this->get_option('removeFailedSubscribers') && !empty($result['results'])) {
                    foreach ($result['results'] as $key => $subscribersSend) {
                        if(isset($subscribersSend['error'])) {
                            $failedSubcribers[] = $data['registration_ids'][$key];
                        }
                    }
                }
            }
        ...
        if($this->get_option('removeFailedSubscribers') && !empty($failedSubcribers)) {
            $guest_tokens = $this->token_manager->getGuestTokens();
            $active_tokens = array_values( array_diff($guest_tokens, $failedSubcribers) );
            update_option('fire_push_guest_tokens', $active_tokens); 
        }
